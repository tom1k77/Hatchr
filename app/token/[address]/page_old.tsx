...
export default async function TokenPage(props: { params: { address?: string } }) {
  // ВАЖНО: забираем address только из props.params
  const rawAddress = props?.params?.address;
    ...
    return (
      <div className="hatchr-root">
        <div className="hatchr-shell">
          <h1 style={{ fontSize: "22px", fontWeight: 600, marginBottom: 8 }}>Token</h1>
          ...
          >
            <div>debug:</div>
            <div>rawAddress =&nbsp;{String(rawAddress ?? "")}</div>
          </div>
  ...
  // Берём все токены напрямую с сервера
  ...
  // Простая MVP-карточка токена
  const shortAddress = `${token.token_address.slice(0, 6)}…${token.token_address.slice(-4)}`;

  return (
    <div className="hatchr-root">
      <div className="hatchr-shell">
        <h1 style={{ fontSize: "22px", fontWeight: 600, marginBottom: 16 }}>
